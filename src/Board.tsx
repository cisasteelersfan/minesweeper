...
export const Board = (): JSX.Element => {
  ...
  const setAdjacent = (b: BoardType): BoardType => {
    const rows = b.length;
  ...
  const generateBoardWithStart = (row: number, col: number): BoardType => {
    const board = Array.from(new Array(ROWS), generateRandomRow);
    for (let r = -1; r <= 1; r++) {
      for (let c = -1; c <= 1; c++) {
  ...
  const uncoverSquare = (row: number, col: number) => (): void => {
    if (!gameStarted) {
      setGameStarted(true);
      const initializedBoard = setAdjacent(generateBoardWithStart(row, col));
